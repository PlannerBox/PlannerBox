...
export class CourseRepository implements ICourseRepository {
  ...
  async findEvents(query: PaginateQuery): Promise<Paginated<Course>> {
    ...
    return await paginate<Course>(query, queryBuilder, {
      relations: {
        teachers: { account: { rolePermissions: true } },
        skills: true,
        group: true,
        room: { useMaterialRoom: { material: true } },
      },
  ...
  async findCourse(id: string): Promise<CourseM> {
    return CourseMapper.fromEntityToModel(
      await this.courseRepository.findOne({ where: { id: id } }),
    );
  }
