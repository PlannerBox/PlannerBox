...
export class GroupManagementController {
  ...
  @ApiOperation({
    description: 'Returns a paginated list of all groups (nestjs pagination)',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiQuery({
    ...
    type: String,
    description:
      'add a filter on the property group.name (check nestjs paginate doc for more details)',
  })
  async getGroupPaginatedList(
    @Paginate() query: PaginateQuery,
  ): Promise<Paginated<Group>> {
    return await this.getGroupUsecasesProxy
      .getInstance()
      .findGroupPaginatedList(query);
  }
  ...
  async getGroupDetails(
    ...
    const group = await this.getGroupUsecasesProxy
      .getInstance()
      .findGroupDetails(groupId);
  ...
  async getPaginatedGroupDetailsByAccount(
    @Paginate() query: PaginateQuery,
  ): Promise<any> {
    return await this.getGroupUsecasesProxy
      .getInstance()
  ...
  @ApiResponse({
    status: 200,
    description:
      'Returns a summary list of all users (used to add a member to a group)',
  ...
  async getAllUsersSummary(): Promise<NestedAccountDto[]> {
    const accounts = await this.addMemberUseCase
      .getInstance()
      .getAllSummaryAccounts();
  ...
  async addMember(
    @Param('groupId') groupId: string,
    @Body() newGroupMemberDto: NewGroupMemberDto,
  ): Promise<any> {
    await this.addMemberUseCase
      .getInstance()
      .addMember(groupId, newGroupMemberDto);
